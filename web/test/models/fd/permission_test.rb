...
class Fd::PermissionTest < ActiveSupport::TestCase
  ...
  test "the three roles stack, the work then the rules then the tool itself" do
    ...
    assert_equal 19, Fd::Permission.held_by("lead").size
    assert_equal 23, Fd::Permission.held_by("community_manager").size
    assert_equal Fd::Permission.keys.size, Fd::Permission.held_by("community_manager").size

    assert_equal %w[decision.settle decision.retire access.read access.grant app.flip
                    engine.run engine.sync].sort,
      Fd::Permission.lead_only.sort
    assert_equal %w[access.read access.grant app.flip engine.sync].sort,
      Fd::Permission.manager_only.sort
  end
