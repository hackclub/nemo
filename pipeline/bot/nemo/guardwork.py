...
def took_it_further(client, conn, guard, user_id, ts):
    gid, _kind, channel_id, thread_ts, _state, opened_by, warned_ts, _expires = guard

    if warned_ts is None or ts <= warned_ts:
        return None
    if guards.exempt(conn, user_id, opened_by):
        return None

    try:
        remove(client, channel_id, ts)
    except Exception as failure:
        log.warning("nemo: guard %s could not remove %s: %s", gid, ts, failure)

    count, already = guards.strike(conn, gid, user_id)
    ...
    if cap and guards.punished(conn, gid) >= cap:
        log.warning("nemo: guard %s hit the reset cap of %s, leaving %s alone", gid, cap, user_id)
        return None

    return guards.reset_for(conn, gid, user_id, channel_id, thread_ts)
